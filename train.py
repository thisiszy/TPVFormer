...
from utils.plot import plot_point_cloud
import mmcv
...
def main(local_rank, args):
    ...
    if cfg.resume_from and osp.exists(cfg.resume_from):
        map_location = "cpu"
        ckpt = torch.load(
            cfg.resume_from, map_location=map_location, weights_only=False
        )
    ...
    while epoch < max_num_epochs:
        ...
        for i_iter, (
            ...
            ce_loss = loss_func(ce_input, ce_label)
            loss = lovasz_loss + ce_loss

            loss_dict_tmp = {
                "lovasz_loss": lovasz_loss.detach().cpu().numpy(),
            ...
            loss_dict = OrderedDict(
                {
                    k: (loss_dict.get(k, 0) * i_iter + v) / (i_iter + 1)
            ...
            if i_iter % print_freq == 0 and dist.get_rank() == 0:
                ...
                if experiment is not None:
                    experiment.log_metric(
                        "train/lovasz_loss", loss_dict["lovasz_loss"], step=global_iter
                    )
                    experiment.log_metric(
                        "train/ce_loss", loss_dict["ce_loss"], step=global_iter
                    )
                    experiment.log_metric(
        ...
        if dist.get_rank() == 0:
            ...
            save_file_name = os.path.join(
                os.path.abspath(args.work_dir), f"epoch_{epoch+1}.pth"
            )
        ...
        with torch.no_grad():
            for i_iter_val, (
                ...
                ce_loss = loss_func(ce_input.detach(), ce_label)
                loss = lovasz_loss + ce_loss
                loss_dict_tmp = {
                    "lovasz_loss": lovasz_loss.detach().cpu().numpy(),
                    "ce_loss": ce_loss.detach().cpu().numpy(),
                ...
                val_loss_dict = OrderedDict(
                    {
                        k: (val_loss_dict.get(k, 0) * i_iter_val + v) / (i_iter_val + 1)
                        for k, v in loss_dict_tmp.items()
                    }
                )  # update mean losses
                ...
                if i_iter_val % print_freq == 0 and dist.get_rank() == 0:
                    logger.info(
                        ...
                        % (epoch, i_iter_val, loss.item(), loss_dict["loss"])
                    )
        ...
        if experiment is not None:
            experiment.log_metric(
                "val/lovasz_loss", val_loss_dict["lovasz_loss"], step=global_iter
            )
            experiment.log_metric(
                "val/ce_loss", val_loss_dict["ce_loss"], step=global_iter
            )
            experiment.log_metric(
                "val/total_loss", val_loss_dict["loss"], step=global_iter
            )
        ...
        logger.info("Current val loss is %.3f" % (np.mean(val_loss_dict["loss"])))
