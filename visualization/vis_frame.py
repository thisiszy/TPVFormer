...
def draw(
    ...
    pred_vox_dataframe = pd.DataFrame(
        grid_coords_pred, columns=["x", "y", "z", "label"]
    )
    ...
    fig.update_layout(
        ...
        height=800,
        width=2400,  # Adjusted width for three plots
        scene=dict(
            ...
            aspectmode="cube",
        ),  # Important for equal aspect ratio
        scene2=dict(
            bgcolor="rgba(0,0,0,0)",
            xaxis=dict(range=range_val),
            yaxis=dict(range=range_val),
...
def main():
    ...
    from visualization.dataset import (
        ImagePoint_NuScenes_vis,
        DatasetWrapper_NuScenes_vis,
        ImagePoint_FLINK_vis,
    )
    ...
    for index in args.frame_idx:
        ...
        pt_coords = torch.from_numpy(np.stack([pt_coords]).astype(np.float32)).to(
            device
        )
        with torch.no_grad():
            outputs_vox, outputs_pts = my_model(
                img=imgs, img_metas=[img_metas], points=pt_coords.clone()
            )
        ...
        grid_size = cfg.grid_size
        resolution = np.array(
            [(e - s) / l for e, s, l in zip(voxel_max, voxel_origin, grid_size)],
            dtype=np.float32,
        )
        ...
        for filename in filelist:
            if os.path.exists(filename):
                shutil.copy(
                    filename, os.path.join(frame_dir, os.path.basename(filename))
                )
            else:
