...
def build(
    ...
    if dataset_config["dataset_type"] == "ImagePoint_NuScenes":
        ...
        train_dataset = ImagePoint_NuScenes(
            data_path, imageset=train_imageset, label_mapping=label_mapping, nusc=nusc
        )
        val_dataset = ImagePoint_NuScenes(
            data_path, imageset=val_imageset, label_mapping=label_mapping, nusc=nusc
        )
        ...
        train_dataset = ImagePoint_FLINK(
            train_data_path,
            label_mapping=label_mapping,
            label_name=label_name,
            img_num=train_dataloader_config["img_num"],
        )
        val_dataset = ImagePoint_FLINK(
            val_data_path,
            label_mapping=label_mapping,
            label_name=label_name,
            img_num=val_dataloader_config["img_num"],
        )
    else:
